...
def eligible_for_pool(row, pool, finishing_order):
    ...
    For pool "place": bets on the winner (if Win or Place) or on second (if Place) are eligible.
    For pool "show": bets on the winner (all bet types) are eligible; bets on the runner‑up qualify if bet as Place (or Win) or Show; bets on third qualify if bet as Show.
    """
...
if not st.session_state.bets.empty:
    ...
    if finish_order:
        ...
        # Compute pool payout with full allocation of the pool.
        def compute_pool_payout(df, pool, pool_total, eligible_mask, contrib_col, fallback_mask):
            ...
            # Compute remainder and distribute it proportionally among eligible bets.
            ...
            # Scale final payouts so that they sum to pool_total.
        ...
        # For fallback masks here we simply use the eligible mask.
