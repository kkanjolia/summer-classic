...
def eligible_for_pool(row, pool, finishing_order):
    ...
    For pool "show": bets on the winner; on second if bet as Place/Show; on third if bet as Show.
    """
...
if not st.session_state.bets.empty:
    ...
    if finish_order:
        ...
        # --- New payout function that mirrors the win refund logic for place and show ---
        def compute_pool_payout(df, pool, pool_total, eligible_mask, contrib_col, fallback_mask):
            ...
            # Scaling adjustment so that the sum equals pool_total.
            distributed = df[pool + "_payout_raw"].sum()
            scale_factor = pool_total / distributed if distributed != 0 else 1
            df[pool + "_payout_final"] = df[pool + "_payout_raw"] * scale_factor
            df[pool + "_payout_final"].fillna(0, inplace=True)
            return df
        ...
        st.dataframe(final_df[[
            ...
            "win_payout_final", "place_payout_final", "show_payout_final",
            "Final Payout"
        ]])
