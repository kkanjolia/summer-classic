...
for key in ["current_user", "admin_logged_in", "wagering_closed", "finishing_order"]:
    if key not in st.session_state:
        st.session_state[key] = None if key=="current_user" or key=="finishing_order" else False
...
def effective_contribution(bet_type, amount, pool_category):
    ...
    Splits each bet into contributions for each pool:
      - Win bet: contributes amount/3 to the win, place, and show pools.
      - Place bet: contributes amount/2 to the place and show pools.
      - Show bet: contributes 100% to the show pool.
    """
    if bet_type == "Win":
        return amount / 3.0
    elif bet_type == "Place":
...
def eligible_for_pool(row, pool, finishing_order):
    """
    Returns True if a wager qualifies for the given pool based on the finishing order.
      - "win": only win bets on the finishing-order winner qualify.
      - "place": wagers on the winner (if bet as Win or Place) or on the runner‑up (if bet as Place) qualify.
      - "show": wagers on the winner (all types) qualify; wagers on the runner‑up qualify if bet as Place or Show; wagers on third qualify if bet as Show.
    """
...
if not st.session_state.bets.empty:
    ...
    if finish_order:
        ...
        # Compute eligible total contributions.
        ...
        # Helper function to compute pool payout.
        def compute_pool_payout(df, pool, pool_total, eligible_mask, contrib_col, fallback_mask):
            if df.loc[eligible_mask, contrib_col].sum() > 0:
                ratio = pool_total / df.loc[eligible_mask, contrib_col].sum()
            ...
            # Due to rounding, adjust so that the sum equals pool_total.
            distributed = df[pool + "_raw"].sum()
            if distributed != 0:
                scale_factor = pool_total / distributed
            else:
                scale_factor = 1
            df[pool + "_payout"] = df[pool + "_raw"] * scale_factor
