...
def eligible_for_pool(row, pool, finishing_order):
    ...
    For pool "place": bets on the winner (if Win or Place) OR on the runner‑up (if Place).
...
if not st.session_state.bets.empty:
    ...
    if finish_order:
        ...
        # Revised function: distribute remainder to ALL eligible bets (not just those with zero raw payout).
        def compute_pool_payout_adjusted_v3(df, pool, pool_total, finish_order):
            if pool == "win":
                ...
                fallback_mask = df.apply(lambda r: (r["Betting On"] == winner and r["Bet Type"] in ["Win", "Place"]) or 
                ...
                fallback_mask = df.apply(lambda r: (r["Betting On"] == winner) or 
            ...
            # Use eligible mask if there are any eligible bets; otherwise, fallback.
            ...
            if total_eff > 0:
                ...
                df[pool + "_raw"] = df.apply(lambda r: r[contrib_col] * ratio if mask.loc[r.name] else 0, axis=1)
            else:
                df[pool + "_raw"] = 0
            
            total_claimed = df[pool + "_raw"].sum()
            remainder = pool_total - total_claimed
            if remainder > 0 and mask.sum() > 0:
                total_weight = df.loc[mask, contrib_col].sum()
                if total_weight > 0:
                    extra_ratio = remainder / total_weight
                    df[pool + "_extra"] = df.apply(lambda r: r[contrib_col] * extra_ratio if mask.loc[r.name] else 0, axis=1)
                else:
                    df[pool + "_extra"] = 0
            else:
                df[pool + "_extra"] = 0
        ...
        df = compute_pool_payout_adjusted_v3(df, "win", total_win, finish_order)
        df = compute_pool_payout_adjusted_v3(df, "place", total_place, finish_order)
        df = compute_pool_payout_adjusted_v3(df, "show", total_show, finish_order)
