...
########################################
# HELPER FUNCTIONS: POOL ELIGIBILITY & EFFECTIVE WAGER
########################################
def effective_contribution(amount):
    """In this simpler model the effective wager is the full bet amount."""
    return amount

def eligible_for_pool(row, pool, finishing_order):
    """
    Returns True if the wager qualifies for the specified pool.
      - For "win": Only if Betting On equals finishing_order["winner"] and Bet Type is "Win".
      - For "place": If Betting On equals finishing_order["winner"] and Bet Type in ["Win", "Place"] OR
                     if Betting On equals finishing_order["second"] and Bet Type is "Place".
      - For "show": If Betting On equals finishing_order["winner"] (all types) OR
                    if Betting On equals finishing_order["second"] and Bet Type in ["Place", "Show"] OR
                    if Betting On equals finishing_order["third"] and Bet Type is "Show".
    """
...
if not st.session_state.bets.empty:
    ...
    # Now we compute individual pool payouts using the pure pari-mutuel method.
    # The effective wager for a bet is defined as its full wager if eligible.
    # Payout ratio = (pool total) / (sum of effective wagers for eligible bets).
    # If no eligible wagers exist, then the pool is refunded proportionally among all bets in that category.
    ...
    if finish_order:
        ...
        # Compute effective wagers for each pool.
        ...
        # Show pool: For the winner, all bets qualify; for runner-up if bet as "Place" or "Show"; for third if bet as "Show".
        ...
        if total_eff_win > 0:
            ...
            # Refund all win bets proportionally.
        ...
        # Final payout is the sum across pools.
        df["Final Payout"] = df["win_payout"] + df["place_payout"] + df["show_payout"]
        
        # Filter out wagers with zero final payout.
        final_df = df[df["Final Payout"] > 0].copy()
        
        st.header("Individual Payouts (Final)")
        st.markdown("Final breakdown per wager (only showing bets that receive a positive payout):")
