...
########################################
# PUBLIC BET FORM & ADMIN OPTIONS
########################################
# Admin login & deletion controls (unchanged)
...
if not st.session_state.bets.empty:
    ...
    # For payout calculations we use a simplified parimutuel formula:
    # For each pool, only eligible bets contribute their full wager amount.
    # Then payout ratio = (Pool Total) / (Sum of effective wagers for eligible bets)
    # Then a bet’s payout from that pool = its effective wager * ratio.
    
    df = st.session_state.bets.copy()
    
    # We assume finishing order is set by admin.
    finishing_opts = [
        "Anthony Sousa", "Connor Donovan", "Chris Brown", "Jared Joaquin",
        "Jim Alexander", "Joe Canavan", "Mark Leonard", "Pete Koskores",
    ...
    if finish_order:
        ...
        # Define effective wager per pool:
        # Win Pool: Only if bet type == "Win" and horse == winner.
        df["eff_win"] = df.apply(lambda r: r["Bet Amount"] if (r["Betting On"]==winner and r["Bet Type"]=="Win") else 0, axis=1)
        # Place Pool: For winner if bet type in ["Win", "Place"], and for second if bet type=="Place".
        ...
        # Show Pool: For winner (all types), for second if bet type in ["Place","Show"], for third if bet type=="Show".
        ...
        # Determine payout ratios per pool (if no one is eligible in win pool, refund all win bets)
        if total_eff_win > 0:
            win_ratio = total_win / total_eff_win
        else:
            # No eligible win bets on winner, so refund all win bets (losing bets) proportionally.
            win_mask = df["Bet Type"]=="Win"
            total_eff_win = df.loc[win_mask, "Bet Amount"].sum()
        ...
        # For each bet, compute individual pool payouts.
        df["win_payout"] = df["eff_win"] * win_ratio
        df["place_payout"] = df["eff_place"] * place_ratio
        df["show_payout"] = df["eff_show"] * show_ratio
        
        # Final payout for the bet is the sum across the pools.
        df["Final Payout"] = df["win_payout"] + df["place_payout"] + df["show_payout"]
        
        # (Optionally, you can create columns for detailed breakdown)
        df["Raw Payout"] = df["Final Payout"]  # In this method, all funds are fully allocated.
        
        # Filter out rows with zero final payout.
        final_df = df[df["Final Payout"] > 0].copy()
        
        st.header("Individual Payouts (Final)")
        st.markdown("Below is a breakdown per wager (only showing bets that receive a final payout):")
