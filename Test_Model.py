...
# Set session_state.current_user if real name chosen
if user_choice != "Select a name...":
    st.session_state.current_user = user_choice
else:
    st.session_state.current_user = None

# Show user name (if any)
...
def effective_contribution(bet_type, amount, category):
    """Split each bet into Win/Place/Show according to each-way logic."""
    if bet_type == "Win":
...
#   Public Bet Form
########################################
# Show the bet form only if wagering is open and user is selected
...
########################################
#   If we have bets, show pool sizes, finishing order, payouts
########################################
if len(st.session_state.bets)>0:
    ...
    def create_summary():
        ...
        # rename columns for Win/Place/Show
    ...
    if winner:
        ...
        # If any pool has zero winners, redistribute
        data_pools = [
