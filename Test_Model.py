...
if not st.session_state.bets.empty:
    ...
    if finish_order:
        ...
        # --- NEW FUNCTION: Compute pool payout using same logic for all pools.
        def compute_pool_payout_adjusted_v4(df, pool, pool_total, eligible_mask, contrib_col, fallback_mask):
            total_eff = df.loc[eligible_mask, contrib_col].sum()
            if total_eff > 0:
                ratio = pool_total / total_eff
                df.loc[eligible_mask, pool + "_final"] = df.loc[eligible_mask, contrib_col] * ratio
            else:
                total_fb = df.loc[fallback_mask, "Bet Amount"].sum()
                if total_fb > 0:
                    ratio = pool_total / total_fb
                    df.loc[fallback_mask, pool + "_final"] = df.loc[fallback_mask, "Bet Amount"] * ratio
                else:
                    df[pool + "_final"] = 0
            df[pool + "_final"].fillna(0, inplace=True)
            return df
    
        # For fallback, we use the same eligible mask.
        df = compute_pool_payout_adjusted_v4(df, "win", total_win, win_mask, "Win Contrib", win_mask)
        df = compute_pool_payout_adjusted_v4(df, "place", total_place, place_mask, "Place Contrib", place_mask)
        df = compute_pool_payout_adjusted_v4(df, "show", total_show, show_mask, "Show Contrib", show_mask)
