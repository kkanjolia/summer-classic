...
def effective_contribution(bet_type, amount, pool_category):
    """
    In a pure pari-mutuel, if a wager qualifies for a pool it contributes its full bet amount.
    """
    return amount

def eligible_for_pool(row, pool, finishing_order):
    """
    Determines whether a wager qualifies for a pool payout.
      - For "win": only wagers with Bet Type "Win" on the winning horse qualify.
...
if not st.session_state.bets.empty:
    ...
    # Use pure pari-mutuel method.
    # For each pool, only wagers that are eligible contribute their full bet amount.
    # Payout ratio = (Pool Total) / (Sum of effective wagers among eligible bets)
    # Individual pool payout = effective wager × ratio.
    # If no eligible wager exists (sum == 0), refund the entire pool proportionally among all bets in that pool.
    ...
    if finish_order:
        ...
        # Place pool: Wagers on the winner qualify if bet type in ["Win", "Place"]; wagers on the runner-up qualify if bet type is "Place".
        ...
        # Show pool: Wagers on the winner qualify (any type); on the runner-up qualify if bet type in ["Place","Show"];
        # on the third-place qualify if bet type is "Show".
        ...
        # Final payout is the sum of all pool payouts.
        ...
        st.markdown("Final breakdown per wager (only displaying bets that receive a positive payout):")
