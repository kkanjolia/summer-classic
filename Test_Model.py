...
def effective_contribution(bet_type, amount, pool_category):
    """
    Splits each bet into contributions for Win/Place/Show pools as follows:
...
# Show one banner: if wagering is closed, show closed message; else, open.
...
########################################
#  Pool Calculations & Detailed Wager Summary
########################################
if not st.session_state.bets.empty:
    ...
    def create_summary():
        ...
        summary.columns.name = None  # remove any hierarchy
    ...
    if winner:
        ...
        # Start with full effective pools
        wpool_eff = tw_eff
        ppool_eff = tp_eff
        spool_eff = ts_eff
        ...
        # Calculate final payout ratios (pure pari-mutuel, no scaling)
        ...
        # Final payout calculation per bet based on finishing order and bet type.
        ...
        st.header("Individual Payouts (Pure Pari-Mutuel)")
